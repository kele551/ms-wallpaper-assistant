# 微软壁纸助手

**让电脑桌面壁纸自己换，你不用管。**

Windows 桌面壁纸自动轮换工具。图全都来自微软官方，两个图源：

| 图源 | 说明 |
|---|---|
| **必应每日一图** | 微软必应每天换的那张，4K 原图 |
| **Windows 聚焦** | 微软官方聚焦图库，一次能刷一批，也是 4K |

它有几个地方跟别的壁纸软件不一样：

- **不用安装** —— 下载一个 exe，双击就用
- **不要管理员权限**，不写注册表，不装计划任务
- **后台换图时一个窗口都不闪**，你根本感觉不到它在跑
- **几天没开机也不漏图** —— 错过的必应壁纸会自动补齐
- **图片都存在你自己电脑上**，不上传任何东西

---

## 下载

### 👉 [点这里下载最新版](https://github.com/kele551/ms-wallpaper-assistant/releases/latest)

下载下来是一个 zip，解压后里面就两样东西：

```
微软壁纸助手.exe     ← 双击这个
使用说明.txt         ← 一页纸说明
```

就这一个 exe，不用挑、不用装、不用下一步下一步。

---

## 三步用起来

**第 1 步** —— 解压，双击 `微软壁纸助手.exe`，菜单就出来了。

**第 2 步** —— 第一次打开，它会自动下载今天的必应图 + 一批聚焦图，屏幕上会告诉你壁纸存在哪。等几十秒就好。

**第 3 步** —— 想让它以后开机自己换，菜单里按一下 **[A]**。

```
想立刻看效果？菜单里按 [1]，马上换一张。
```

> 按 `[A]` 不用管理员。它只是在 Windows「启动」文件夹里放一个快捷方式而已。

---

## 菜单上都有什么

```
—— 换图 ——
 [1] 立刻换一张 (聚焦)
 [2] 换上今天的必应每日一图
 [3] 再抓一批新聚焦
—— 壁纸库 ——
 [4] 从库里挑一张
 [5] 随便来一张
 [6] 打开壁纸文件夹
—— 补图 ——
 [7] 补下最近错过的必应 (几天没开机)
 [8] 下载往期必应 (2021 年至今, 4K)
—— 其他 ——
 [F] 收藏当前这张 / 取消收藏
 [S] 设置
 [A] 打开开机自动换壁纸   (现在是「关」的时候出现)
 [B] 关掉开机自动换壁纸   (现在是「开」的时候出现)
 [L] 查看运行日志
 [Q] 退出
```

按 `[S]` 进设置，能改这几样：**多久换一张**（默认 30 分钟）、**壁纸存哪**、
**填充方式**（填充 / 适应 / 拉伸 / 居中 / 平铺 / 跨区）、**收藏夹**。

看到喜欢的图，主菜单按 `[F]` 就收下了。收藏够了还能设成「只在收藏里轮换」，
以后换出来的都是你自己挑过的图。

---

## 壁纸存哪？能自己删吗？

默认存在**「图片」文件夹里的「壁纸」**，下面两个子目录：

```
<图片文件夹>\壁纸\必应     必应每日一图
<图片文件夹>\壁纸\聚焦     Windows 聚焦
```

如果你的「图片」在 C 盘，它会挪到别的盘去 —— 壁纸天天攒，几年下来好几个 GB，
放系统盘容易把 C 盘撑满。

**库里的图你随便删、随便挪、随便拷走，程序完全不受影响：**

| 你做的事 | 它的反应 |
|---|---|
| 删掉几张 | 不再排进轮换，也不会被重新下载回来 |
| 整个库挪走 | 菜单会提醒你，到设置里把位置指过去就行 |
| 正在用的那张被移走了 | 如实写出来，换一张就更新了 |

---

## 常见问题

| 问 | 答 |
|---|---|
| **双击没反应？** | 多半是安全软件拦了，把它加入信任就行。程序只从 bing.com 和微软官方接口下图，没有任何上传行为。 |
| **中文显示成乱码？** | 系统「非 Unicode 程序语言」要设成「中文(简体, 中国)」。 |
| **会不会拖慢电脑？** | 只有「开机自动换」开着时后台才有一个进程，平时占 30 MB 左右，绝大多数时间在睡觉。关掉就完全不占。 |
| **提示某个盘「写不进去」？** | 那是这台机器那个盘的权限设置（常见于用第三方分区工具格的盘），不是程序不支持它。按提示选 `[1]` 一键修好，或者换个盘 —— 壁纸放哪个盘都能用。 |
| **不想用了怎么卸？** | 菜单按 `[B]` 关掉自动换 → 删掉 exe → 想连配置也清掉就再删数据目录（`[S]` 设置 `[7]` 里能看到位置）。**下载的壁纸一张都不会被删。** |

---

## 用了之后，跟我说说感受

好不好用、哪儿别扭、还想要什么功能 —— 都欢迎说，一句两句也行：

- **发邮件**：75219857@qq.com
- **开 Issue**：https://github.com/kele551/ms-wallpaper-assistant/issues

顺手带上这几样，能省一轮来回：

1. Windows 版本（设置 → 系统 → 关于）
2. 程序版本号（菜单标题上就有，比如 `v1.5.2`）
3. `wallpaper.log` 末尾十几行（菜单 `[L]` 能看到）

---

## 版本迭代

每次改版都会打一个版本号、发一版下载。**当前最新：v1.5.3**

| 版本 | 日期 | 改了什么 |
|---|---|---|
| **v1.5.3** | 09-19 | 看过的壁纸不会再排回来（另记一份「看过」名单）；按 `[A]` 开自动换后**当场**就生效，不用等下次开机 |
| v1.5.2 | 09-19 | 「开机自动换」拆成 `[A]` 打开 / `[B]` 关掉两个键，菜单上只摆当前能按的那一个，不会再按反 |
| v1.5.1 | 09-19 | 只出一份 exe，不用挑该点哪个；第一次打开不再追着问你壁纸存哪 |
| v1.5.0 | 09-19 | 配置和收藏搬到用户目录，exe 旁边不再多出任何文件夹 |
| v1.4.2 | 09-19 | 修：桌面快捷方式删掉后，又会自己冒出来 |
| v1.4.1 | 09-19 | 桌面快捷方式只自动建一次，之后你把它收进哪个文件夹，它就跟着在哪 |
| v1.4.0 | 09-19 | 壁纸填充方式可以自己选了；新增收藏夹；自动建桌面图标；换了新的程序图标 |
| v1.3.0 | 09-18 | 壁纸默认存到「图片\壁纸」；图片被删掉或移走后照常工作 |
| v1.2.3 | 09-18 | 选位置时所有盘都列出来，写不进去的当场一键修好 |
| v1.2.1 | 09-18 | 修：保存位置老是落到同一个盘；第一次运行可以自己挑 |
| v1.2.0 | 09-18 | 改成绿色单文件版：双击即用、不安装、不要管理员、后台零窗口 |
| v1.1.0 | 09-18 | 单层菜单、启动不闪黑窗、错过的必应图自动补 |
| v1.0.0 | 09-17 | 第一个版本 |

逐条改动记录见 [CHANGELOG.md](CHANGELOG.md)。

> **一句说明**：v1.2.0 以前是「安装版」（要双击 bat 装计划任务），那个做法已经作废。
> 现在的版本是绿色单文件，老的安装脚本不再提供 —— 直接下最新版就行。

---

## 许可

MIT
